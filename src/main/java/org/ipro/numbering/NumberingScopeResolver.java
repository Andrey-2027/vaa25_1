 ...
 * работает со строками-ключами — ровно как RLS работает с именами измерений. Встроенная
 * реализация по умолчанию ({@link RlsScopeResolver}) достаёт значение из
 * {@code RlsDimensionValue.getRlsChecks()} там, где scope нумерации совпадает с измерением
 * доступа; сущность без RLS-измерения может предоставить свой резолвер.</p>
 */
@FunctionalInterface
