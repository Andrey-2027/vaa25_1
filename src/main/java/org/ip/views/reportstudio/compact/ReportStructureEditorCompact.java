package org.ip.views.reportstudio.compact;
 ...
 */
public class ReportStructureEditorCompact extends VerticalLayout {
    ...
    public ReportStructureEditorCompact() {
        setPadding(false);
        setSpacing(false);
        setSizeFull();
        ...
        VerticalLayout flowPane = flowPane();
        SplitLayout inner = new SplitLayout(fieldsPanel(), palettePanel());
        inner.setSplitterPosition(48);
        inner.setSizeFull();
        inner.getStyle().set("min-height", "0");

        SplitLayout leftSplit = new SplitLayout(flowPane, inner);
        leftSplit.setSplitterPosition(26);
        leftSplit.setSizeFull();
        leftSplit.getStyle().set("min-height", "0");
        add(leftSplit);
        setFlexGrow(1, leftSplit);
    }
    ...
    private void configureFields() {
        ...
        fieldsGrid.addComponentColumn(ReportStructureEditorCompact::kindCell).setHeader("Вид").setAutoWidth(true);
        fieldsGrid.addComponentColumn(ReportStructureEditorCompact::fieldLabelCell).setHeader("Поле / текст").setFlexGrow(1);
    ...
    private void configurePalette() {
        ...
        paletteKind.setItemLabelGenerator(ReportStructureEditorCompact::kindLabel);
    ...
    private void refreshFlowLane() {
        ...
        for (int i = 0; i < ordered.size(); i++) {
            ...
            flowLane.add(flowCard(band));
            if (i < ordered.size() - 1) {
                Icon arrow = new Icon(VaadinIcon.ARROW_DOWN);
                arrow.setSize("12px");
                arrow.getStyle().set("color", "var(--lumo-contrast-30pct)").set("margin", "1px auto").set("display", "block");
                flowLane.add(arrow);
            }
        }
    }
    ...
    private com.vaadin.flow.component.Component flowCard(ReportBand band) {
        ...
        HorizontalLayout card = new HorizontalLayout(icon, kind, meta);
        card.setWidthFull();
        ...
        card.getStyle().set("border", "1px solid " + (band == selectedBand ? "var(--lumo-primary-color-50pct)" : "var(--lumo-contrast-10pct)"))
                ...
                .set("cursor", "pointer").set("gap", "var(--lumo-space-s)");
