 ...
 * валидация до обработчика, Failure без исключений наружу, конфликт @Version
 * → отдельный исход Conflict (не Failure), честный doSave(),
...
class ItemFormSaveFlowTest {
    ...
    void saveWrapsHandlerExceptionInFailure() {
        ...
        assertThat(result).isNotInstanceOf(FormSaveResult.Conflict.class);
        assertThat(result.success()).isFalse();
        assertThat(((FormSaveResult.Failure<TestDocument>) result).messages())
    ...
    @Test
    void saveTranslatesSpringOptimisticLockFailureIntoConflict() {
        ItemForm<TestDocument> form = formWithMetadata();
        ObjectOptimisticLockingFailureException boom =
            new ObjectOptimisticLockingFailureException(TestDocument.class, 7L);
        FormSaveHandler<TestDocument> handler = mock(FormSaveHandler.class);
        when(handler.save(form)).thenThrow(boom);
        form.setSaveHandler(handler);
        ...
        assertThat(result).isInstanceOf(FormSaveResult.Conflict.class);
        assertThat(result.success()).isFalse();
        FormSaveResult.Conflict<TestDocument> conflict =
            (FormSaveResult.Conflict<TestDocument>) result;
        assertThat(conflict.cause()).isSameAs(boom);
        assertThat(conflict.messages()).anyMatch(m -> m.contains("другим пользователем"));
    }

    @Test
    void saveTranslatesJpaOptimisticLockExceptionIntoConflict() {
        ItemForm<TestDocument> form = formWithMetadata();
        jakarta.persistence.OptimisticLockException boom =
        ...
        assertThat(result).isInstanceOf(FormSaveResult.Conflict.class);
        assertThat(result.success()).isFalse();
        assertThat(((FormSaveResult.Conflict<TestDocument>) result).cause()).isSameAs(boom);
    }
