...
### 2.10. Исправление ревью D3.4

Ревью D3.4 признало срез корректным (физическая граница, ownership конфигураций, registration-файл,
POM и manifest согласованы), но нашло одно замечание и два риска. Замечание закрыто до физического
переноса: иначе D3.5 переносил бы уже известный дефект.

**Замечание P2: заменяемость CRUD-бинов не работала.** `CrudAutoConfiguration` регистрировала
четыре concrete-класса через `@Import` и обещала в javadoc, что «повторное определение того же
типа просто выигрывает у импорта». Это неверно для Spring: `@Import` не участвует в
conditional-обработке, поэтому бин другого имени давал двух кандидатов одной инъекции, а того же
имени — конфликт definition overriding. Четыре бина объявлены явными `@Bean
@ConditionalOnMissingBean` по прецеденту `MetadataAutoConfiguration`; `@PersistenceContext` в
`ReferenceCheckService` продолжает работать, потому что BeanPostProcessor'ы применяются и к бинам,
созданным фабричным методом. Заменяемость проверяется двумя тестами
(`applicationBeansReplaceAllPlatformCrudBeans`, `replacingOneBeanKeepsTheOthersPlatformRegistered`),
а не комментарием. Сохранено и то, что было контрактом, а не backoff: `LookupService` по-прежнему
требует `CanonicalReadExecutor` безусловно, и это проверяет отдельный тест.

У исправления есть измеримое следствие, которое стоит назвать: явная проводка сделала видимыми
четыре ребра `org.ipro.crud.config -> {data, metadata, metadata.config, rls}`. Они существовали и
раньше — их скрывал `@Import`, который называет только импортируемый класс, а не его зависимости.
Измерение стало честнее; рёбра внесены в reviewed allow-list `platform-package-matrix.txt` с
причиной. Порядок применения зафиксирован `@AutoConfigureBefore(MetadataAutoConfiguration.class)`:
раньше он держался на алфавитной сортировке FQN — признаке, случайном для требования.

**Риск P2: singleton хранит состояние конкретного Vaadin UI.** `FormCoordinator` хранит mutable
`WorkspaceGateway` и `FormOpenMode`, а `setWorkspace()` вызывается из prototype-view. В
многопользовательском приложении последний вызов направляет навигацию в `Workspace` чужого UI.
Лечение (UI-scope координатора, `WorkspaceGateway` из UI-scoped контекста, удаление `setWorkspace`
из views) — работа D3.5, и она зафиксирована решением, а не отложена: до публикации
`platform-vaadin` координатор не имеет права оставаться singleton'ом с чужим UI внутри.

**Риск P2: глобальный `TelemetryBridge` меняется без lifecycle cleanup.** Конструктор
Vaadin-листенера пишет `EventSink` в статический `TelemetryBridge`, хотя
`TelemetryAutoConfiguration` делает то же самое; при закрытии контекста значение не снимается.
Регистрация переходит lifecycle-компоненту владельца `platform-telemetry` с compare-and-clear при
destroy, а UI-листенер оставляет за собой только Vaadin error handling. Тоже работа D3.5.

**Усиление гейта (неблокирующее замечание).** `AutoconfigureModuleCompositionTest` искал `org.ip`
и `com.vaadin` только в строках `import`, поэтому fully-qualified ссылка обошла бы правило — ровно
та дыра, которую D1 закрывал в своём заборе. Сканер приведён к общей методике: import + wildcard +
fully-qualified, комментарии вырезаны, строковые литералы сохранены. Отдельно добавлен самозамер
`forbiddenReferenceScannerSeesAllThreeSyntaxes`: гейт, который никогда не находил нарушения, ничего
не доказывает.

### 2.11. D3.5.0 — измерение до переноса

Первый шаг D3.5 по плану — не создание каталога модуля, а фиксация измерения: 90 production-типов
будущего `platform-vaadin` (`org.ipro.form` 80, `org.ipro.vaadin` 7, `org.ip.telemetry.vaadin` 3)
получают по одной семантической роли до того, как начнётся переезд. Реестр —
`src/test/resources/platform-vaadin-surface.txt`, забор — `PlatformVaadinSurfaceTest` в дереве
приложения (модуля ещё нет, смотреть нужно на каталоги-кандидаты).

Почему порядок именно такой. Альтернатива — перенести 83 public-класса и зафиксировать
получившуюся поверхность целиком — уже была опробована в D3.3 и дала `TEMPORARY_ALL_PUBLIC`:
состав зафиксирован, а решение о том, что является контрактом, не принято ни по одному типу.
Здесь роли назначаются, пока решение по каждому типу ещё можно принять, а не подтвердить
задним числом.

Бюджет ролей: `APP_API` 28, `APP_SPI` 12, `MODULE_API` 0, `INTERNAL` 50. Пустой `MODULE_API` —
измеренный факт: сегодня ни один `platform-*` модуль не называет тип из этих трёх корней.
Единственный внешний потребитель будущего модуля — Report (`org.ipro.form.EntityField`,
`org.ipro.form.SelectionFormAssembler`), и оба типа уже `APP_API`.

Пять legacy-типов D1-реестра получили явное решение, а не унаследовали роль из правила:
`FormCoordinator -> INTERNAL` (повышение до API заморозило бы именно ту поверхность, которую
D3.5 обязан расчистить — `ApplicationContext`, `ServiceLocator`, mutable `Workspace`),
`ItemTable`, `ListForm`, `EntitySummary`, `EntitySummaryAssembler`, `SubsystemSummaryAssembler` ->
`APP_API` (тип, который платформа передаёт в SPI-точку, обязан быть публичным API, иначе SPI
нереализуем; explorer DTO читаются прикладными views и по плану переезжают в модуль). Решения
записаны в шапке реестра вместе с причиной.

Три свойства проверяются, и все три — про расхождение, а не про наличие списка: полнота (роль у
каждого типа, лишняя запись тоже ломает сборку), самосогласованность файла (число записей под
заголовком `# ---- ROLE (n)` совпадает с бюджетом в нём, запись под чужим заголовком — ошибка) и
замкнутый словарь ролей (черновиковая `APP_API_DRAFT` не является решением). Плюс забор
не-вакуумный: набор типов обязан содержать 90, иначе проверка «роль у каждого» выполнялась бы
на пустом множестве.

Черновик реестра воспроизводится скриптом `node scripts/analyze-platform-surface.mjs
--vaadin-registry`; решения по спорным типам приняты ревью и записаны в файле — иначе
проверялось бы то же правило, которым список построен.

Попутно закрыт тот же класс ошибки, что D3.3 описал в §2.8: широкое правило `*.txt` в
`.gitignore` исключило бы новый reviewed-реестр из поставки, и гейт защищал бы только машину
автора. Исключение добавлено — это четвёртый случай за D-этапы, когда причиной была не ошибка
гейта, а отсутствующее исключение.
...
D3.0–D3.4 закрыты вместе с замечанием ревью (§2.10); D3.5 открыт первым шагом — измерением
до переноса (§2.11). Дальше по плану: D3.5.1–D3.5.3 (публичные интерфейсы над Lookup/ServiceLocator,
очистка `FormContext`, UI-scope координатора) — подготовительные изменения, которые обязаны
предшествовать физическому переносу; затем D3.5.4–D3.5.8 (`platform-vaadin`), D3.6 (консолидация
редакторов), D3.7 (report-модули), D3.8 (BOM + starters + переключение приложения), D3.9
(governance и документация) — с долгами из §2.7.

Порядок внутри D3.5 не случаен и следует из §2.10: пока `FormCoordinator` остаётся singleton'ом с
mutable `Workspace`, а `FormContext` раскрывает `ApplicationContext` и concrete `LookupService`,
любой перенос зафиксирует эти утечки как «платформенную норму». Поэтому подготовительные изменения
идут раньше переезда, а не вместе с ним.

Уже сделанное упрощает перенос вдвое: `platform-core` публикует роли (`MODULE_API` — это и есть
перечень того, что нужно autoconfigure), `EntityDataAccess` повышен до `APP_API`, поэтому
`JpqlDatasetRunner` и прикладные адаптеры остаются реализациями приложения, а не переносятся в
модуль; и теперь роли назначены всем 90 типам будущего модуля, поэтому переезд не зацементирует
текущую поверхность.
