...
- ⏳ Дальше: `settings` (замыкание уже пусто) → пара `telemetry` + `rls` (цикл по одному
  классу в каждую сторону — сначала вывернуть связь через контракт) → `reportstudio`
  последним, после консолидации трёх редакторов отчётов.

Гейты моста: `mvn verify` — 1359/0/0, random-order — 1359/0/0 (seed `20260915`).
