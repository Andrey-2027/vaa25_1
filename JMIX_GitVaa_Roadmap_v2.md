...
Артефакт: [`docs/architecture/d3-subsystem-extraction.md`](docs/architecture/d3-subsystem-extraction.md).

Между «граница существует» (D2) и адаптерами (D3) оставался неотвеченный вопрос: подсистема
платформы действительно может жить вне дерева или это удалось только с типами без бинов,
сущностей и рефлексии. Порядок ответа задан **замером замыкания** (типы дерева, достижимые
транзитивно от пакета подсистемы): `numbering` — 1, `settings` — 2, `telemetry` — 1,
`rls` — 17, `reportstudio` — 42.

- ✅ **Срез 5 — `platform-metadata` (2 типа).** Весь упор подсистем в метаданные оказался
  сканером и индексом ссылок, а не пакетом `org.ipro.metadata` (37 классов). Попутно сняты
  два мёртвых default-значения `org.ip` — реестр строковых связок сократился с 13 файлов до
  11, и забор теперь сканирует также исходники артефактов.
- ✅ **Срез 6 — `platform-numbering` (17 типов).** Первый модуль-подсистема: сущности,
  репозитории, шесть бинов, своя авто-конфигурация, `@EntityScan`/`@EnableJpaRepositories`,
  аннотации, которые читает рефлексия с сущностей приложения, и сохранённое направление
  «rls → numbering». Приложение не потеряло и не приобрело ни одной регистрации.
- ✅ **Срез 7 — `platform-settings` (8 типов).** Подсистема констант целиком: сущность с
  репозиторием, каталог, сервис, источник обратных ссылок и своя авто-конфигурация. Срез
  ценен не выносом, а тем, что его не подтвердил ни один тест модуля: две регрессии
  (`SettingsServiceSliceIT`, `RlsIntegrationTest`) нашёл полный прогон, обе — про срез,
  который имитирует регистрацию модуля вместо её подключения. Из этого выросло общее
  правило в `PersistenceTypeRegistrationTest`, которое выводит модули из их собственных
  объявлений и больше не требует правки при следующем выносе.
- ✅ **Шаг 8а — цикл `rls` ↔ `telemetry` вывернут.** Замер замыкания, задавший порядок работ,
  оказался заниженным: он читал только `import` и не видел ни ссылок полным именем в теле, ни
  однопачечных зависимостей посещённого типа. Честный замер дал для телеметрии **32** типа
...
Начинается после моста D2 → D3: адаптерам нужны подсистемы, умеющие объявлять себя сами,
иначе стартер собирал бы инфраструктурный граф за них. Условия входа в шаги моста и
зарегистрированные задачи (`platform.subsystem-scan-package` в конфигурацию приложения,
свод `IdentifiableEntity`/`BaseEntity`, консолидация редакторов отчётов) — в
